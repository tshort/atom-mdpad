...
module.exports =
class MdpadPreviewView extends ScrollView
  ...
  @deserialize: (state) ->
    new MdpadPreviewView(state)

  @content: ->
    @div class: 'mdpad-preview native-key-bindings', tabindex: -1
  ...
  handleEvents: =>
    ...
    if @editor?
      # if not atom.config.get("mdpad.triggerOnSave")
  ...
  renderHTMLCode: (text) ->
    # if not atom.config.get("mdpad.triggerOnSave") and @editor.getPath()? then @editor.save()
  ...
  showLoading: ->
    @html $$$ ->
      @div class: 'atom-html-spinner', 'Loading mdpad preview\u2026'
